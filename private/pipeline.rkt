...
(provide >>> (struct-out exn:fail:hasket))
...
(struct exn:fail:hasket exn:fail (position) #:constructor-name make-exn:fail:hasket)
(define (errorR->hasket-exn e)
  (define exception (errorR-exception e))
  (make-exn:fail:hasket (exn-message exception) (exn-continuation-marks exception) (reverse (position-value (errorR-position e)))))
;; ------------------------------------------
...
(define-syntax-parse-rule (>>> val:expr step:expr ...)
  (let ((result ((pipeline (unitP val) step ...) (init))))
...
(module+ test
  ...
  (check-true (exn:fail:hasket? (>>> 0 (lambda (v) (errorP (exn (format "~a" v) (current-continuation-marks)))))))
  (check-true (>>> 0 ($ (lambda (v) (unitP (exn:fail:hasket? v)))) (lambda (v) (errorP (exn (format "~a" v) (current-continuation-marks))))))
  (>>> 0
       ($ (lambda (v) (unitP (check-equal? '(1) (exn:fail:hasket-position v)))))
